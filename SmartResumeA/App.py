...
def scrape_naukri_jobs(skills):
    url = f'https://www.naukri.com/{("-".join(skills)).replace(" ","-")}-jobs?k={("%2C%20".join(skills)).replace(" ","%20")}&nignbevent_src=jobsearchDeskGNB'
    print(url)
    ...
    target_divs = soup.find_all('div', class_='srp-jobtuple-wrapper')

    # Iterate through each found div
    for target_div in target_divs:
        # Extract data from the current div
        title = target_div.select_one('.title').text.strip()
        company_name = target_div.select_one('.comp-name').text.strip()
        experience = target_div.select_one('.exp-wrap .expwdth').text.strip()
        salary = target_div.select_one('.sal-wrap .ni-job-tuple-icon-srp-rupee').text.strip()
        job_description = target_div.select_one('.job-desc').text.strip()
        job_url = target_div.select_one('.title')['href']

        # Check if the job description contains any of the specified skills
    ...
    df.to_html('job_data.html', index=False)

    print("Data dumped to job_data.html.")
    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                scrape_naukri_jobs(resume_data['skills'])
                st.success("Scraping complete! Check the job_data.html file for results.")
                if st.button("Open HTML File"):
                    webbrowser.open(r'C:\Users\hemac\OneDrive\Desktop\SmartResumeA\job_data.html')
                ##  recommendation
